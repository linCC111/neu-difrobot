...
class AlohaEnv(gym.Env):
    ...
    def __init__(
        ...
        elif self.obs_type == "pixels_agent_ee_pos":
            self.observation_space = spaces.Dict(
                {
                    "pixels": spaces.Dict(
                        {
        ...
        if self.task == "transfer_cube" or self.task == "insertion":
            self.action_space = spaces.Box(low=-1, high=1, shape=(len(ACTIONS),), dtype=np.float32)
        elif self.task == "end_effector_transfer_cube" or self.task == "end_effector_insertion":
            self.action_space = spaces.Box(low=-1, high=1, shape=(ee_pos_len,), dtype=np.float32)
    ...
    def reset(self, seed=None, options=None):
        ...
        if self.task == "transfer_cube":
            ...
            EE_BOX_POSE[0] = sample_box_pose(seed)
        elif self.task == "insertion":
            BOX_POSE[0] = np.concatenate(sample_insertion_pose(seed))  # used in sim reset
        elif self.task == "end_effector_insertion":
            EE_BOX_POSE[0] = np.concatenate(sample_insertion_pose(seed))
        else:
