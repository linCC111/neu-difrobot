...
EE_BOX_POSE = [None]  # to be changed from outside
...
class BimanualViperXEndEffectorTask(base.Task):
    ...
    def get_ee_pos(physics):
        ...
        left_qpos_raw = qpos_raw[:8]
        right_qpos_raw = qpos_raw[8:16]
        left_ee_pos = np.concatenate([physics.data.mocap_pos[0], physics.data.mocap_quat[0]]).copy()
        right_ee_pos = np.concatenate([physics.data.mocap_pos[1], physics.data.mocap_quat[1]]).copy()
        left_gripper_qpos = [normalize_puppet_gripper_position(left_qpos_raw[6])]
        right_gripper_qpos = [normalize_puppet_gripper_position(right_qpos_raw[6])]
        return np.concatenate([left_ee_pos, left_gripper_qpos, right_ee_pos, right_gripper_qpos])
    ...
    def get_observation(self, physics):
        ...
        obs["ee_pos"] = self.get_ee_pos(physics)
        obs["images"] = {}
...
class TransferCubeEndEffectorTask(BimanualViperXEndEffectorTask):
    ...
    def initialize_episode(self, physics):
        ...
        if EE_BOX_POSE is not None:
            cube_pose = EE_BOX_POSE[0]
        else:
...
class InsertionEndEffectorTask(BimanualViperXEndEffectorTask):
    ...
    def initialize_episode(self, physics):
        ...
        if EE_BOX_POSE is not None:
            peg_pose, socket_pose = EE_BOX_POSE[0][:8], EE_BOX_POSE[0][8:16]
        else:
